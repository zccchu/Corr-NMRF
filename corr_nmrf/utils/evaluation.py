...
from corr_nmrf.utils.logger import log_every_n_seconds
...
class DispEvaluator(DatasetEvaluator):
    ...
    def __init__(self, thres, only_valid, min_disp=None, max_disp=None, eval_prop=True, divis_by=8):
        """
        Args:
            thres (list[str] or None): threshold for outlier
            only_valid (bool): whether invalid pixels are excluded from evaluation
            min_disp (int or None): If None, minimum disparity will be regarded as -infinity
        ...
        self._min_disp = -np.inf if min_disp is None else min_disp
    ...
    def process(self, inputs, outputs):
        ...
        for input, output in zip(inputs, outputs):
            ...
            if self._only_valid:
                valid = valid_gt & (disp_gt >= self._min_disp) & (disp_gt < self._max_disp)
            else:
                valid = (disp_gt >= self._min_disp) & (disp_gt < self._max_disp)
            ...
            denom = torch.abs(disp_gt.flatten()[val]).clamp_min(1e-6)
            self._d1.append(((epe[val] > 3) & (epe[val] / denom > 0.05)).float().mean().item())
            ...
            if self._eval_prop:
                if 'super_pixel_label' not in input:
                    continue
                ...
                mask = (((mini_disp_gt >= self._min_disp) & (mini_disp_gt < self._max_disp)).sum(dim=-1)) > 0.5
    ...
    def evaluate(self):
        ...
        if self._eval_prop and len(self._prop_epe) > 0:
