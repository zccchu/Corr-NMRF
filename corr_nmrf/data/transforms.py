...
class FlowAugmentor:
    def __init__(self, crop_size, min_scale=-0.2, max_scale=0.5, do_flip=True, yjitter=False, saturation_range=[0.6,1.4], gamma=[1,1,1,1], random_crop=True, spatial_crop=True):
    ...
    def spatial_transform(self, img1, img2, flow):
        ...
        if self.yjitter:
            if self.random_crop:
                ...
                y0 = int(np.clip(
                    (img1.shape[0] - self.crop_size[0]) // 2,
                    2, max(2, img1.shape[0] - self.crop_size[0] - 2)))
                x0 = int(np.clip(
                    (img1.shape[1] - self.crop_size[1]) // 2,
                    2, max(2, img1.shape[1] - self.crop_size[1] - 2)))
                y1 = y0
            ...
            if self.random_crop:
                ...
                y0 = max(0, (img1.shape[0] - self.crop_size[0]) // 2)
                x0 = max(0, (img1.shape[1] - self.crop_size[1]) // 2)
            img1 = img1[y0:y0 + self.crop_size[0], x0:x0 + self.crop_size[1]]
...
class SparseFlowAugmentor:
    def __init__(self, crop_size, min_scale=-0.2, max_scale=0.5, do_flip=False, yjitter=False, saturation_range=[0.7,1.3], gamma=[1,1,1,1], random_crop=True, spatial_crop=True):
        # spatial augmentation params
        self.crop_size = crop_size
        self.spatial_crop = spatial_crop
        self.random_crop = random_crop
    ...
    def spatial_transform(self, img1, img2, flow, valid):
        ...
        margin_y = 20
        margin_x = 50
        h_max = img1.shape[0] - self.crop_size[0]
        w_max = img1.shape[1] - self.crop_size[1]
        if self.random_crop:
            y0 = np.random.randint(0, img1.shape[0] - self.crop_size[0] + margin_y)
            x0 = np.random.randint(-margin_x, img1.shape[1] - self.crop_size[1] + margin_x)
            y0 = np.clip(y0, 0, h_max)
            x0 = np.clip(x0, 0, w_max)
        else:
            y0 = int(np.clip((img1.shape[0] - self.crop_size[0]) // 2, 0, h_max))
            x0 = int(np.clip((img1.shape[1] - self.crop_size[1]) // 2, 0, w_max))
