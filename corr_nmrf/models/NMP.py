...
from corr_nmrf.models.corr_nmrf_nmp import CorrNMRFNMP
...
class SwinNMP(nn.Module):
    ...
    def forward(self, label_rep, abs_encoding, attn_mask, stereo_context=None):
        """
...
class Propagation(nn.Module):
    ...
    def __init__(
        ...
        if prop_type in ("rw_ttt", "da2_rw_ttt"):
            if rw_ttt is None:
                raise ValueError(f"prop_type='{prop_type}' requires rw_ttt module (replaces CSWin)")
            if len(layers) > 0:
                raise ValueError(f"prop_type='{prop_type}' must not use CSWin PropagationLayer")
    ...
    def forward(self, cost_volume, label_seed, context, prob=None):
        """
        cost_volume: B*H*W,G,D, groupwise cost volume
        coord_seed: [B*H*W, num_seed], integer disparity modals
        context:    [ B,H,W,C], visual context
        prob:       [B*H*W, D], optional DPN disparity distribution for RW-TTT
        """
        # extract cost of label seeds
        N = label_seed.shape[-1]
        seed_idx = label_seed.long()
        cost = Propagation.sample_cost(cost_volume, seed_idx)
        ...
        if self.rw_ttt is not None:
            if prob is None:
                raise ValueError("RW-TTT requires prob from DPN (gather seed confidence)")
            seeds_conf = torch.gather(prob, dim=-1, index=seed_idx)
            seeds_cost = cost.min(dim=-1).values
            spatial_shape = (context.shape[0], context.shape[1], context.shape[2])
            sample_embed = self.rw_ttt(
                sample_embed,
                label_seed,
                seeds_cost,
                seeds_conf,
        ...
        intermediate = []
        if self.prop_type == "cswin" and len(self.layers) > 0:
...
class Inference(nn.Module):
    ...
    @staticmethod
    def _pad_bhw_n_c(x, ht, wd, top_pad, down_pad, left_pad, right_pad):
        x = rearrange(x, '(b h w) n c -> b h w n c', h=ht, w=wd)
        x = F.pad(x, (0, 0, 0, 0, left_pad, right_pad, top_pad, down_pad))
        x = rearrange(x, 'b h w n c -> (b h w) n c')
        return x

    @staticmethod
    def _pad_bhw_n(x, ht, wd, top_pad, down_pad, left_pad, right_pad):
        x = rearrange(x, '(b h w) n -> b h w n', h=ht, w=wd)
        x = F.pad(x, (0, 0, left_pad, right_pad, top_pad, down_pad))
        x = rearrange(x, 'b h w n -> (b h w) n')
        return x
    ...
    def forward(self, labels, fmap1, fmap2, fmap1_gw, fmap2_gw):
        ...
        left_fmap = repeat(fmap1, 'b c h w -> b c h w n', n=N)
        feat_concat = torch.cat((left_fmap, warped_fmap2), dim=1)
        feat_concat = rearrange(feat_concat, 'b c h w n -> (b h w) n c')
        label_rep = self.ffn(torch.cat((feat_concat, corr), dim=-1))
        left_feat_ctx = rearrange(left_fmap, 'b c h w n -> (b h w) n c')
        right_feat_ctx = rearrange(warped_fmap2, 'b c h w n -> (b h w) n c')
        ...
        if H_pad > 0 or W_pad > 0:
            label_rep = self._pad_bhw_n_c(label_rep, ht, wd, top_pad, down_pad, left_pad, right_pad)
            abs_encoding = self._pad_bhw_n_c(abs_encoding, ht, wd, top_pad, down_pad, left_pad, right_pad)
            left_feat_ctx = self._pad_bhw_n_c(left_feat_ctx, ht, wd, top_pad, down_pad, left_pad, right_pad)
            right_feat_ctx = self._pad_bhw_n_c(right_feat_ctx, ht, wd, top_pad, down_pad, left_pad, right_pad)
            corr_ctx = self._pad_bhw_n_c(corr, ht, wd, top_pad, down_pad, left_pad, right_pad)
            labels_ctx = self._pad_bhw_n(labels, ht, wd, top_pad, down_pad, left_pad, right_pad)
        else:
            corr_ctx = corr
            labels_ctx = labels

        # Build shifted-window masks only when any layer still uses SwinNMP.
        if any(isinstance(layer.nmp, SwinNMP) for layer in self.layers):
        ...
        stereo_context = {
            "left_feat": left_feat_ctx,
            "right_feat": right_feat_ctx,
            "corr": corr_ctx,
            "labels": labels_ctx,
            "stage": "inference",
        }
...
class Refinement(Inference):
    ...
    def forward(self, labels, fmap1, fmap2, fmap1_gw, fmap2_gw):
        ...
        label_rep = self.ffn(torch.cat((feat_concat, corr), dim=-1))
        left_feat_ctx = rearrange(fmap1, 'b c h w -> (b h w) 1 c')
        right_feat_ctx = rearrange(warped_fmap2, 'b c h w -> (b h w) 1 c')
...
class InferenceLayer(nn.Module):
    def __init__(self, embed_dim, mlp_ratio, window_size, shift_size, n_heads,
                 activation="gelu", attn_drop=0., proj_drop=0., drop_path=0., dropout=0., normalize_before=False,
                 nmp_type="swin", ttt_rank=8, ttt_base_lr=1.0, ttt_mini_batch_size=16, ttt_max_chunks=32,
                 ttt_hard_token_ratio=0.5, ttt_use_spatial_fusion=True, ttt_fusion_kernels=(1, 3, 5),
                 ttt_dropout=0.0, ttt_use_diag=True, ttt_target_mode="epipolar",
                 ttt_stop_grad_target=True, ttt_use_confidence_weight=True, ttt_use_residual=True, cost_group=32,
                 ttt_epipolar_target_style="split_ln", ttt_corr_lambda=0.5, ttt_loss_type="smooth_l1",
                 ttt_smooth_l1_beta=1.0, ttt_router_loss_weight_conf=True):
        super().__init__()
        ...
        if nmp_type == "swin":
            self.nmp = SwinNMP(
                embed_dim, qk_dim, num_heads=n_heads, window_size=window_size,
        ...
        elif nmp_type in ("corr_nmrf", "spatial_ttt_v2"):
            self.nmp = CorrNMRFNMP(
                dim=embed_dim,
                qkv_dim=qk_dim,
                num_heads=n_heads,
                rank=ttt_rank,
                mini_batch_size=ttt_mini_batch_size,
                max_chunks=ttt_max_chunks,
                base_lr=ttt_base_lr,
                hard_token_ratio=ttt_hard_token_ratio,
                use_spatial_fusion=ttt_use_spatial_fusion,
                fusion_kernels=ttt_fusion_kernels,
                dropout=ttt_dropout,
                mlp_ratio=mlp_ratio,
                normalize_before=normalize_before,
                target_mode=ttt_target_mode,
                stop_grad_target=ttt_stop_grad_target,
                use_confidence_weight=ttt_use_confidence_weight,
                use_diag=ttt_use_diag,
                drop_path=drop_path,
                cost_group=cost_group,
                use_residual=ttt_use_residual,
                epipolar_target_style=ttt_epipolar_target_style,
                corr_lambda=ttt_corr_lambda,
                ttt_loss_type=ttt_loss_type,
                smooth_l1_beta=ttt_smooth_l1_beta,
                router_loss_weight_conf=ttt_router_loss_weight_conf,
            )
        else:
            raise ValueError(f"Unknown nmp_type: {nmp_type}")

    def forward(self, tgt, abs_encoding, attn_mask, ht, wd, stereo_context=None):
        """
        tgt: B*H*W,N,C
        ...
        tgt = self.nmp(tgt, abs_encoding=abs_encoding, attn_mask=attn_mask, stereo_context=stereo_context)
        return tgt
...
class RefinementLayer(nn.Module):
    def __init__(self, dim, mlp_ratio, window_size, shift_size, n_heads,
        ...
        if nmp_type == "swin":
            self.nmp = SwinNMP(
                dim, qk_dim, num_heads=n_heads, window_size=window_size,
                shift_size=shift_size, mlp_ratio=mlp_ratio, attn_drop=attn_drop,
                drop_path=drop_path, drop=dropout, act_layer=act_layer, normalize_before=normalize_before
            )
        elif nmp_type in ("corr_nmrf", "spatial_ttt_v2"):
