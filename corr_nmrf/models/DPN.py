...
from corr_nmrf.models.NMP import MLP, Propagation, PropagationLayer
...
class DPN(nn.Module):
    ...
    def __init__(self, cost_group, num_proposals, feat_dim, context_dim, num_prop_layers,
        ...
        if prop_type in ("rw_ttt", "da2_rw_ttt"):
            # RW-TTT / DA2-RW-TTT fully replaces CSWin PropagationLayer(s).
        ...
        self.propagation = Propagation(
            prop_embed_dim, cost_group, layers=prop_layers,
            norm=prop_norm, return_intermediate=False, rw_ttt=rw_ttt,
            prop_type=prop_type,
        )
    ...
    def from_config(cls, cfg):
        if cfg.NMP.PROP_TYPE != "cswin":
            raise ValueError("This Corr-NMRF release keeps the CSWin-style proposal propagation used by whu_stereo_resnet.yaml")
        return {
    ...
    def forward(self, cost_volume, fmap1_list):
        ...
        memory, label_seeds = self.propagation(cost_volume, label_seeds, context, prob=prob)
        outputs = self.prop_head(memory).view(-1, *label_seeds.shape)
        labels = torch.clamp(outputs + label_seeds[None], min=0.0, max=float(nd - 1))  # candidate labels in index domain
